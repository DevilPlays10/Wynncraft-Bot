...
async function player(interaction) {
    ...
    return WynGET(`player/${name}?fullResult`, BFSToken(interaction.guildId, interaction.member) ? `Bearer ${tokens.wyn_api_GUILD}` : null).then(async res => {
        ...
        const guildSearched = guild?.data?.members?.[dat.guild.rank.toLowerCase()]?.[dat.username]
        ...
        ints[msg.resource.message.id] = {
            ...
            pages: [
                new EmbedBuilder()
                    ...
                    .setDescription(`\`\`\`ex\n${page1["1"].join('\n')}\`\`\``)
                    .addFields([
                        ...page1["5"]? [{ name: 'Guild:', value: `\`\`\`ex\n${page1["5"].join('\n')}\`\`\`` }]: [],
                        { name: "Stats:", value: `\`\`\`ml\n${page1["2"].join('\n')}\`\`\`` },
                        { name: 'Featured:', value: `\`\`\`ex\n${page1["3"].join('\n')}\`\`\`` },
                        ...page1["4"].length? [{ name: 'Username History:', value: `\`\`\`yaml\n${page1["4"].join('\n')}\`\`\`` }]: []
                    ])
                    .setTimestamp()
        ...
        return {
            embeds: [ints[msg.resource.message.id].pages[0]], components: [
