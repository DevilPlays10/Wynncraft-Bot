...
async function guild(interaction) {
    ...
    //end
    return await WynGET(prefix ? `guild/prefix/${guild}` : `guild/${guild}`, isBFSMember(interaction.guildId, interaction.member) ? `Bearer ${tokens.wyn_api_GUILD}` : null).then(async (res) => {
        const dat = res.data
        console.log(dat)
        ...
        const embed2 = new EmbedBuilder()
            ...
            .setDescription(`**Total Members:** \`${dat.members.total}\`\n**Owner:**\n\`\`\`yaml\n${(owner.name).padEnd(16, ' ')} | ${formatNumberShort(owner.xp)} XP ${(new Date() - owner.joined) < 604800000 ? `[${relative(ent.joined, 'dhms', 0, 1)}] ` : ""}${owner.online ? `- (${owner.server})` : ""}\`\`\``)
            .setColor(color)
        for (r of ["chief", "strategist", "captain", "recruiter", "recruit"]) {
            const list = mlist.filter(ent => ent.rank == r).map(ent => `${(ent.name).padEnd(16, ' ')} | ${formatNumberShort(ent.xp)} XP ${(new Date().getTime() - new Date(ent.joined).getTime()) < 604800000 ? `[${relative(ent.joined, 'dhms', 0, 1)}] ` : ""}${ent.online ? `- (${ent.server})` : ""}`)
            if (!list.length) {
                embed2.addFields({
